...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers(HttpMethod.GET, "/", "/eventos").permitAll()
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.userDetailsService(userDetailsService).passwordEncoder(bCryptPasswordEncoder());// gerando uma senha criptografada
	}
	 ...
	 * Exception { auth.inMemoryAuthentication().withUser("root").password("root").
	 * roles("ROOT"); }
	 */
}
