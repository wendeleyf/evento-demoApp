...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests()
			...
			.antMatchers(HttpMethod.GET, "/addEventos", "/{id}").hasRole("ADMIN") // apenas administradores
			.antMatchers(HttpMethod.POST, "/addEventos", "/{id}").hasRole("ADMIN") // apenas administradores
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.userDetailsService(userDetailsService) // gerando autenticação
				.passwordEncoder(new BCryptPasswordEncoder()); // gerando uma senha criptografada
	}
	 ...
	 * Exception {
	 * auth.inMemoryAuthentication().withUser("cleefsouza").password("root123").
	 * roles("ADMIN"); }
	 */
}
